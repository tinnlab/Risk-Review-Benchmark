...
class Net(nn.Module):
	def __init__(self, modalities, m_length, input_modality_dim, n_continuous, embedding_size, fusion_method='attention', device=None):
		"""
		Parameters
		----------
		modalities: list
		...
		if 'clinical' in self.data_modalities:
			self.clinical_submodel = ClinicalEmbeddingNet(m_length=self.m_length, n_continuous=self.n_continuous, embedding_size=self.embedding_size)
