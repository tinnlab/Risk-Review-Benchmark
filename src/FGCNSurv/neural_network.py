...
class Graph_Survival_Analysis(nn.Module):
    ...
    def __init__(self, nmirna, nrna):
        nn.Module.__init__(self)
        self.miRNA_encoding = nn.Sequential(nn.Linear(nmirna, 50),    # original code: in_features = 600
                                            nn.Tanh(),
                                            nn.BatchNorm1d(50),
                                            Highway(50, 3, f=F.relu),
                                            )

        self.RNASeq_encoding = nn.Sequential(nn.Linear(nrna, 50),  # original code: in_features = 6000
                                             nn.Tanh(),
                                             nn.BatchNorm1d(50),
                                             Highway(50, 3, f=F.relu),
                                             )
