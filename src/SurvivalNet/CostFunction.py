...
def cost_func(params, dataset, time, fold):
    ...
    with open('./data/' + dataset + '/Time' + str(time) + '/train_set_' + str(fold), 'rb') as f:
        train_set = cPickle.load(f)
    with open('./data/' + dataset + '/Time' + str(time) + '/val_set_' + str(fold), 'rb') as f:
    ...
    _, _, val_costs, val_cindices, _, _, _, maxIter = train(pretrain_set,
                                                            train_set, val_set, pretrain_config, finetune_config,
                                                            n_layers,
