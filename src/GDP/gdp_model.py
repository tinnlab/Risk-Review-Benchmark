    ...
    for i, hidden_units in enumerate(hidden_nodes):
        ...
        if layer_index == 1:
            with tf.name_scope("hidden" + str(layer_index)):
                weights = tf.Variable(tf.truncated_normal([FEATURE_SIZE, hidden_units], dtype=tf.float32) / 20,
                                      name="weights")  # divided by 20 to generate smaller ouptut at initial, otherwise exp(output) will be inf
            ...
            with tf.name_scope("hidden" + str(layer_index)):
                weights = tf.Variable(tf.truncated_normal([pre_hidden_units, hidden_units], dtype=tf.float32) / 20,
                                      name="weights")  # divided by 20 to generate smaller ouptut at initial, otherwise exp(output) will be inf
                # biases1=tf.Variable(tf.zeros([hidden_units]),name="biases")
                # hidden1=tf.nn.relu(tf.matmul(features,weights1)+biases1)
...
def loss(inf_output, censors, groups, batch_size, alpha, scale, delta, reg_type="lasso"):
    ...
    hazard_condition_sum = tf.reverse(tf.cumsum(tf.reverse(hazard, [0])),
                                      [0])  # for given i, get sum of theta_j where dates_j>=dates_i
...
def group_lasso(alpha, scale, groups):
    ...
    all_variables = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES)
    all_weights = [x for x in all_variables if re.search("weights", x.name)]
    weights1 = [x for x in all_weights if re.search("hidden1/weights", x.name)][
        0]  # only the input-hidden1 weights were grouped
