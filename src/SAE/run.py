import sys
sys.path.append("./")
# sys.path.append("/data/daotran/Cancer_RP/Benchmark/ReviewPaper_MethodRun/SAE")
...
def run(datPath, resPath):
    ...
    for fold in all_folds:
        print(fold)
        if os.path.exists(resPath + "/SAE/" + fold) == False:
            os.makedirs(resPath + "/SAE/" + fold)
        ...
        for seed in range(1, 11):
            ...
            predTrain.to_csv(resPath + "/SAE" + '/' + fold + '/Train_Res_' + str(seed) + '.csv', sep=',',
                            header=True)
            predVal.to_csv(resPath + "/SAE" + '/' + fold + '/Val_Res_' + str(seed) + '.csv', sep=',',
                        header=True)