...
def run(datPath, resPath, timerecPath):
    ...
    for dataset in alldatasets:
        ...
        if os.path.exists(resPath + "/SAE/" + dataset) == False:
            os.makedirs(resPath + "/SAE/" + dataset)
        if os.path.exists(timerecPath + "/SAE/" + dataset) == False:
            os.makedirs(timerecPath + "/SAE/" + dataset)
        ...
        for current_time in range(1, 6):
            ...
            if os.path.exists(os.path.join(resPath, "SAE", dataset, 'Time' + str(current_time))) == False:
                os.makedirs(os.path.join(resPath, "SAE", dataset, 'Time' + str(current_time)))
            if os.path.exists(os.path.join(timerecPath, "SAE", dataset, 'Time' + str(current_time))) == False:
                os.makedirs(os.path.join(timerecPath, "SAE", dataset, 'Time' + str(current_time)))
            ...
            for fold, (train_idx, val_idx) in enumerate(
                ...
                predTrain.to_csv(resPath + "/SAE" + '/' + dataset + '/Time' + str(current_time) + '/Train_Res_' + str(fold) + '.csv', sep=',',
                                header=True)
                predVal.to_csv(resPath + "/SAE" + '/' + dataset + '/Time' + str(current_time) + '/Val_Res_' + str(fold) + '.csv', sep=',',
                            header=True)
                time_df.to_csv(
                    timerecPath + "/SAE" + '/' + dataset + '/Time' + str(current_time) + '/TimeRec_' + str(
                        fold) + '.csv',
                    sep=',', header=True)