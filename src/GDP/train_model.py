...
def get_data(filename):
    ...
    for index in range(len(header)):
        if (header[index] != 'days' and header[index] != 'dead'):
            # if (index == 0) :
            # group.append(startGroup + 1)
            if ('_' not in header[index]):
                # clinical features
                startGroup += 1
    ...
    for dtype, frac in {"train": 0.6, "eval": 0.2, "test": 0.2}:
        ...
        datasets[dtype]['X'], datasets[dtype]['T'], datasets[dtype]['O'], datasets[dtype]['A'] = dm.calc_at_risk(
            X[index_s:index_e], T[index_s:index_e], O[index_s:index_e])
        index_s = index_e
...
def do_eval(sess, eval_cindex_o, eval_o_file, eval_type, train_steps, loss_pl, inf_out_pl, feature_pl, at_risk_pl,
            date_pl, censor_pl, data_set, isTrain_pl, isTrain):
    """ Run evaluation
    Args:
    ...
    for step in range(eval_steps):
        ...
        if FLAGS.output_evaluation_cindex == 'YES':
            for k in range(FLAGS.batch_size):
                eval_cindex_o.write(eval_type + "\t" + str(dates[k]) + "\t" + str(-info_output[k][0]) + "\t" + str(
                    censors[k]) + "\t" + str(cindex) + "\n")  # TEST: check dates prediction
        cindex_sum += cindex
        loss_sum += loss
        count += 1
...
def run_training():
    ...
    with tf.Graph().as_default():
        ...
        if FLAGS.model == 'NN':
            ...
            inf_output_pl = dm.inference(feature_pl, hidden_nodes, FLAGS.activation, FLAGS.dropout_keep_rate,
                                         isTrain_pl)
        ...
        loss = dm.loss(inf_output_pl, censor_pl, feature_groups, FLAGS.batch_size, FLAGS.alpha, FLAGS.scale,
                       FLAGS.delta, FLAGS.reg_type)
        ...
        for step in range(FLAGS.max_steps):
            ...
            # fill the data
            feed_dict = fill_feed_dict(data_sets.train.next_batch(FLAGS.batch_size), feature_pl, at_risk_pl, date_pl,
                                       censor_pl)
            ...
            if (step + 1) % 5 == 0 or (step + 1) == FLAGS.max_steps:
                ...
                if FLAGS.skip_eval == 'NO':
                    ...
                    do_eval(sess, eval_cindex_output, eval_output, "training_data", step + 1, loss, inf_output_pl,
                            feature_pl, at_risk_pl, date_pl, censor_pl, data_sets.train, isTrain_pl, False)

                    print("Validation data evaluation:")
                    do_eval(sess, eval_cindex_output, eval_output, "validation_data", step + 1, loss, inf_output_pl,
                            feature_pl, at_risk_pl, date_pl, censor_pl, data_sets.validation, isTrain_pl, False)

                    print("Testing data evaluation:")
                    do_eval(sess, eval_cindex_output, eval_output, "testing_data", step + 1, loss, inf_output_pl,
                            feature_pl, at_risk_pl, date_pl, censor_pl, data_sets.test, isTrain_pl, False)
    if FLAGS.skip_eval == 'NO':
        eval_output.close()
        if FLAGS.output_evaluation_cindex == 'YES':
