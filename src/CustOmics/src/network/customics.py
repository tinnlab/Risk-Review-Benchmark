...
class CustOMICS(nn.Module):
    ...
    def fit(self, omics_train, clinical_df, label, event, surv_time, omics_val=None, batch_size=32, n_epochs=30, verbose=False):
        ...
        self.one_hot_encoder = OneHotEncoder(sparse_output=False).fit(encoded_clinical_df.loc[:, label].values.reshape(-1,1))   ### Dao changed "sparse" to "sparse_output"
        # self.one_hot_encoder = OneHotEncoder.fit(encoded_clinical_df.loc[:, label].values.reshape(-1,1))
    ...
    def predict_risk(self, omics_df):
        ...
        return self.survival_predictor(torch.Tensor(z)).cpu().detach().numpy()     #### changed
