...
class ModelCoach:
	...
	def _process_data_batch(self, data, data_label, phase):
		...
		time = data_label['label'][:, 0].to(self.device)
		event = data_label['label'][:, 1].to(self.device)
	...
	def _run_training_loop(self, num_epochs, scheduler, info_freq, log_dir):
		...
		for epoch in range(1, num_epochs+1):
			...
			for phase in ['train', 'val']:   ### ORIGINAL CODE: DAO CHANGED BECAUSE DAO CARE ONLY ABOUT TRAINING
			# for phase in ['train']:
