...
class _BaseModelWithDataLoader:
	def __init__(self, modalities, m_length, dataloaders, input_modality_dim, n_continuous, embedding_size, fusion_method='attention', device=None):
	...
	def _instantiate_model(self, move2device=True):
		...
		self.model = Net(self.data_modalities, self.m_length, self.input_modality_dim, self.n_continuous, self.embedding_size, self.fusion_method, self.device, )
...
class Model(_BaseModelWithDataLoader):
	def __init__(self, modalities, m_length, dataloaders, input_modality_dim, n_continuous, embedding_size, fusion_method='attention', trade_off=0.3, mode='total', device=None):
		super().__init__(modalities, m_length, dataloaders, input_modality_dim, n_continuous, embedding_size, fusion_method, device)
		self.optimizer = Adam
